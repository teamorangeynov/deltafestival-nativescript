...
class TeamMembersService extends __BaseService {
  ...
  static readonly GetTeamMembers_1Path = '/api/TeamMembers/{id}';
   ...
   * @return Success
   */
  GetTeamMembers_1Response(id: number): __Observable<__StrictHttpResponse<TeamMembers>> {
    let __params = this.newParams();
    let __headers = new HttpHeaders();
    let __body: any = null;
   ...
   * @return Success
   */
  GetTeamMembers_1(id: number): __Observable<TeamMembers> {
    return this.GetTeamMembers_1Response(id).pipe(
      __map(_r => _r.body as TeamMembers)
    );
  }
